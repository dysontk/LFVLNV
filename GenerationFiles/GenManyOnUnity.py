...
class Events:
    ...
    def __init__(self, attempt, run_num):
        ...
        for rn in range(run_num):
            output = subprocess.check_output(f"nohup /work/pi_mjrm_umass_edu/LNV_collider/Generated/{attempt}/bin/madevent ./MyFiles/LFVLNV/GenerationFiles/{attempt}_run.dat > MyFiles/LFVLNV/GenerationFiles/logs/{attempt}_{rn}.log 2>&1 &")
    ...
    def findStartingRunNum(self):
        output = subprocess.check_output(f"ls /work/pi_mjrm_umass_edu/LNV_collider/Generated/{self.attempt}/Events/")
    ...
    def checkRunning(self, run_num):
        output = subprocess.check_output(f"ps | grep {self.pid[run_num]}")
    ...
    def findFileName(self, run_num):
        output = subprocess.check_output(f"ls /work/pi_mjrm_umass_edu/LNV_collider/Generated/{self.attempt}/Events/run_{run_num+self.begin_num:02d}/*delphes_events.root")
    ...
    def check_nEvents(self, run_num):
        ...
        output = subprocess.check_output(f"./read_root_file {self.findFileName(run_num)}")
